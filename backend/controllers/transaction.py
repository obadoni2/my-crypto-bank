...
from model.transaction import Transaction, TransactionState
...
import sqlalchemy 
from model.cryto_currency import  CryptoCurrency 
from flask_jwk_extended import (
    jwt_required,
    get_jwt_identify
)
...
def validate_transaction():
    ...
    user_id = get_jwt_identify()["id"]
    user = User.query.get(user_id)
    
    
    if user_otp == user.otp:
        user.otp = "0" # 0znake da je validiran 
...
def create_transction():
    ...
    user = User.query.get(user_id)
    if user_exist(recipient_email) is True: 
        if user_otp !=user.otp: 
            return{"error": "validation failed, please try again"}
        
        if check_resource(user, cryptocurrency,amount) is False:
           return {"error:" " you don't have enought resources for this transfer"} 
        keccak = generate_hash(user, recipient_email,amount)
        transactionb = Transaction(hashID=keccak.hexdigest(), sender=user.email, recipient=recipient_email,amount=amount,
                                   cryptocurrency=cryptocurrency, user_id=user_id, user=user, state=TransactionState.WAITING_FOR_VALUE )
        
        db.session.add(transaction)
        db.session.commit()
        return Response(status=200)
    else:
        return {"error": "User with that email doen't exist"}
    
def check_resource(user, cryptocurrency,amount):
...
def filter_transaction():
    ...
    schema = TransactionSchema(many=True)   # ako vracam visa 
...
def get_transactions(): 
    ...
    user_id = get_jwt_identify()["id"]
    user = User.query.get(user_id)
    #all_transaction = user.transactions
    all_transactions = Transaction.query.all()
    iterator = filter(lambda x:x.state != TransactionState.WAITING_FOR_USER.value and ( 
        x.snder == user.email or x.recipient == user.email), all_transactions)
    # da bi vratio listu ovo je ogre je iterator 
    all_transactions = list(iterator)
    schema = TransactionSchema(many=True) # ako vracam vise 
    results = schema.dump(all_transactions)
    return jsonify(results)

@transaction.rounte("/getTransactionReques") # saom primljene 
@jwt_required()
...
def mining(user_id, transaction_id, crypto_name,amount crypto_name, amount, q1):
    sleep(5 *1)
    basedir = os.path.abspath(os.path.dirname(__file__))
    goald_dir = os.path.abspath(os.path.join(os.getcwd(), "CrytoDB.db"))
    ...
    crpto_currencies = crypto_account.crypto_currencies 
